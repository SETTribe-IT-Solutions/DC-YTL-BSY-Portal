...
<!-- Add more as needed -->
  ...
  $month = $_POST['month']; // e.g., 7
    ...
    ?>
                        <form class="needs-validation" action="opdRegister_fetch.php" method="POST" novalidate>
  <div class="form-row">
...
if(!empty($month) && !empty($year)){
    $date_con = " AND date BETWEEN '$start_date 00:00:00' AND '$end_date 23:59:59' ";
}else{
}
      ...
      $sr = 1;
      $query = mysqli_query($conn, "SELECT * FROM opdRegister WHERE 1 $date_con ORDER BY registerId DESC, id DESC LIMIT 1");
      while ($row = mysqli_fetch_assoc($query)) {
        echo "<tr>";
        echo "<td></td>";
        echo "<td>{$sr}</td>";
        echo "<td><a href='opdRegister.php?edit={$row['id']}' class='btn btn-sm btn-primary'>Edit</a></td>";
        echo "<td>{$row['date']}</td>";
        echo "<td>{$row['taluka']}</td>";
        echo "<td>{$row['village']}</td>";
        echo "<td>{$row['nameAsha']}</td>";
        echo "<td>{$row['contactNumber']}</td>";
        echo "<td>{$row['patientName']}</td>";
        echo "<td>{$row['age']}</td>";
        echo "<td>{$row['Gender']}</td>";
        echo "<td>{$row['address']}</td>";
        echo "<td>{$row['pateintContact']}</td>";
        echo "<td>{$row['aadharCard']}</td>";
        echo "<td>{$row['height']}</td>";
        echo "<td>{$row['weight']}</td>";
        echo "<td>{$row['temp']}</td>";
        echo "<td>{$row['BP']}</td>";
        echo "<td>{$row['medicleSurgery']}</td>";
        echo "<td>{$row['screenigTuberculosis']}</td>";
        echo "<td>{$row['ScreeningHypertension']}</td>";
        echo "<td>{$row['ScreeningDiabetes']}</td>";
        echo "<td>{$row['ScreeningOral']}</td>";
        echo "<td>{$row['ScreeningEye']}</td>";
        echo "<td>{$row['NCD']}</td>";
        echo "<td>{$row['tests']}</td>";
        echo "<td>{$row['patientTreatment']}</td>";
        echo "<td>{$row['ayushManCard']}</td>";
        echo "<td>{$row['patientReferal']}</td>";
...
<script>
  document.getElementById('exportExcel').addEventListener('click', function () {
    ...
    XLSX.writeFile(wb, "OPD Report.xlsx", {
      bookType: 'xlsx',
      cellStyles: true,
      sheetStubs: false
    });
