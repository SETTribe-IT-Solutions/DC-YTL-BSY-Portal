...
<!-- Add more as needed -->
  ...
  $month = $_POST['month']; // e.g., 7
    ...
    ?>
                        <form class="needs-validation" action="ancpncRegister_fetch.php" method="POST" novalidate>
  <div class="form-row">
...
<thead>
  <tr>
    ...
    <th data-priority="1" style="background-color: #007bff !important;" class="text-white">Sr. No</th>
        <th style="background-color: #007bff !important;" class="text-white">Action</th>
...
if(!empty($month) && !empty($year)){
    ...
    SELECT * FROM ancpncRegi WHERE 1 $date_con
...
");
      while ($row = mysqli_fetch_assoc($query)) {
        ...
        echo "<td></td>";
        echo "<td>{$sr}</td>";
         echo "<td><a href='ancpncRegi.php?edit={$row['id']}' class='btn btn-sm btn-primary'>Edit</a></td>";
        echo "<td>{$row['date']}</td>";
